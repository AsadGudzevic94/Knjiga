...
export function parseQuoteItems(text: string): { item: string; price: number }[] {
...
export function analyzeQuote(req: AnalyzeRequest): QuoteAnalysis {
...
export async function runFullAnalysisPipeline(input: PipelineInput): Promise<PipelineOutput> {
  const { quoteText, serviceCategory, zipCode, userId, businessName } = input;
  const region = getRegionalFactor(zipCode);

  // Step 1: Check cache
  const cached = await findCachedAnalysis(quoteText, serviceCategory, zipCode);
  ...
  const ruleBasedAnalysis = analyzeQuote({
    quoteText,
    serviceCategory,
    zipCode,
    businessName,
  ...
  // Step 2.5: Similar cache match — use cached AI + fresh rule-based
  ...
  // Step 3: AI analysis
  const aiResult = await getAIAnalysis(quoteText, serviceCategory, zipCode, ruleBasedAnalysis);
